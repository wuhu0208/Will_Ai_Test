...
哪些列明型号例外？其中哪些型号若误用回油节流会产生异常高压风险？为什么
原则上不应在同一回路中同时对复动和单动执行元件进行速度控制？

### Standard Answer

一般复动夹紧器的夹紧侧和释放侧都采用回油节流。`LKE`、`LSE`、`TLA`、
`TLB`、`TMA`、`TLV`、`TMV`、`TTA` 例外，两侧采用进油节流。PDF 明确警告
`TLA`、`TLB`、`TMA`、`TLV`、`TMV`、`TTA` 若使用回油节流，回路内可能产生
异常高压，导致夹紧器漏油或损坏；该页对 `LKE` 和 `LSE` 仅要求参照各自产品页，
未将这一后果明确适用于它们。复动与单动执行元件原则上不在同一回路中进行
速度控制，否则单动侧可能释放异常或释放时间过长。

### Scoring Standard

- P1 [15]: 说明一般复动夹紧器两侧均采用回油节流。
- P2 [15]: 完整列出八个进油节流例外型号。
- P3 [15]: 说明八个例外型号两侧采用进油节流。
- P4 [15]: 将回油节流异常高压警告准确限定于 `TLA`、`TLB`、`TMA`、`TLV`、`TMV`、`TTA`。
- P5 [10]: 说明这六个型号误用回油节流可使回路产生异常高压。
- P6 [5]: 说明异常高压可能引起夹紧器漏油。
- P7 [5]: 说明异常高压可能造成夹紧器损坏。
- P8 [10]: 说明单动与复动执行元件原则上不共用同一速度控制回路。
- P9 [10]: 说明混用会使单动侧释放异常或释放时间过长。

### Accepted Variants

- `回油节流` 可写为 `meter-out`；`进油节流` 可写为 `meter-in`。
- 八个例外型号和六个后果适用型号的列举顺序不限，但两组范围必须明确区分。

### Forbidden Errors

- 将一般规则与八个例外对调。
- 将异常高压、漏油或损坏后果扩大到 `LKE`、`LSE` 或全部八个例外型号。
- 声称单动与复动元件可无条件共用同一速度控制回路。

### Tolerance

- All eight meter-in exceptions and the exact six-model consequence scope are required.
...
- Local scope path: 通用液压附录 > 复动夹紧器的速度控制回路 > 八个进油节流例外 / TLA、TLB、TMA、TLV、TMV、TTA 回油节流警告；混用单动与复动夹紧器
- Evidence type: STATE_DIAGRAM + TEXT
- Evidence: 附录规定一般复动夹紧器两侧回油节流，列出八种进油节流例外；异常高压、漏油或损坏警告仅列 TLA、TLB、TMA、TLV、TMV、TTA，LKE/LSE 另指向各自产品页；混用回路会造成单动夹紧器释放异常或过慢。
...
- All ten independently scored safety controls are required.
