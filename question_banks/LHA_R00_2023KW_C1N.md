...
- Evidence type: TEXT
- Evidence: 液压油一览表指定使用相当于ISO粘度等级ISO VG 32的一般液压油。
...
回路中的异物或切削屑会导致漏油或动作不良，因此配管、管接头及配件油孔在投入使用前必须彻底清洁，施工时也必须保持作业环境清洁并防止异物进入回路。

### Scoring Standard

- P1 [25]: 回路中的异物或切削屑会导致漏油
- P2 [25]: 回路中的异物或切削屑会导致动作不良
- P3 [25]: 配管、管接头及配件油孔在使用前彻底清洁
- P4 [25]: 施工时保持作业环境清洁并防止异物进入回路
...
- Evidence type: TEXT
- Evidence: 配管、管接头及配件上的油孔等部位必须彻底清洁干净后方可投入使用；回路中的异物或切削屑等会导致漏油或动作不良；配管施工时应清洁作业环境并防止异物进入回路。
...
- Evidence type: TEXT
- Evidence: 缠绕密封胶带时应留出接头顶部1至2个螺纹牙；残留在回路内的密封胶带头会导致漏油或动作不正常等故障。
...
- Evidence type: TEXT
- Evidence: LKE、LSE、TLA、TLB、TMA、TLV、TMV、TTA的夹紧侧和释放侧均应设置为进油节流回路；使用回油节流会产生异常高压，导致夹紧器漏油或损坏。
...
- Evidence type: TEXT + PROCEDURE
- Evidence: 拆卸装置时应切断压力源和电源，并确认油压、气压回路的压力均为零后再进行拆卸作业。
...
- Evidence type: TEXT
- Evidence: 为防止造成人身伤害，严禁接触动作中的夹紧器，否则会导致手指夹伤等伤害。
...
- Evidence type: TEXT
- Evidence: 应定期检查配管、安装螺栓、螺母、固定环和夹紧器有无松动现象，并应及时加固。
...
- Evidence type: TEXT
- Evidence: 产品保修期为从本厂发货后1年半或开始使用后1年内，以较短者为准。
...
- Evidence type: TEXT
- Evidence: 保修期间因本公司责任发生的故障或不良现象，由本公司负责故障部分的更换或修理。
...
- Evidence type: TEXT
- Evidence: 未按规定定期检查维护；操作人员判断失误或使用不当；用户或第三方不当使用；非本公司产品质量原因；自行或未经本公司同意改造、修理；自然灾害等非本公司责任；磨损老化备件及其更换费用，均不属于保修范围。
...
- Evidence type: TEXT
- Evidence: 因本公司产品故障造成的间接损失不在质保范围之内。
