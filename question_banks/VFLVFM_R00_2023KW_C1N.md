...
- P1 [13]: `VFL` 正确解释为弹簧定位、油压释放系列。
- P2 [13]: 主体尺寸 `2` 正确对应 φ8～φ11 工件孔。
- P3 [13]: 设计编号正确写为 `0`。
- P4 [13]: `080` 正确对应 φ8H8 工件孔。
- P5 [12]: `D` 正确解释为基准定位用基准销。
- P6 [12]: `H20` 正确解释为 20 mm 着座高度。
- P7 [12]: `M` 正确解释为释放动作确认型。
- P8 [12]: `R` 正确解释为供气口 R 位置。
...
- P1 [13]: `VFM` 正确解释为油压定位、油压释放系列。
- P2 [13]: 主体尺寸 `6` 正确对应 φ26～φ30 工件孔。
- P3 [13]: 设计编号正确写为 `0`。
- P4 [13]: `300` 正确对应 φ30H8 工件孔。
- P5 [12]: `C` 正确解释为单一方向定位用菱形销。
- P6 [12]: `H30` 正确解释为 30 mm 着座高度。
- P7 [12]: `B` 正确解释为着座确认型。
- P8 [12]: `L` 正确解释为供气口 L 位置。
...
- P1 [25]: VFL 正确写为弹簧力定位。
- P2 [25]: VFL 正确写为油压释放、油压单动型。
- P3 [25]: VFM 正确写为油压定位。
- P4 [25]: VFM 正确写为油压释放、油压复动型。
...
- P1 [13]: VFL 释放时供给释放油压。
- P2 [13]: VFL 释放时活塞/杆上升并使锥套缩径。
- P3 [12]: VFL 撤除释放油压后由弹簧使锥套扩径定位。
- P4 [13]: VFM 释放前撤除定位油压。
- P5 [13]: VFM 释放时供给释放油压。
- P6 [12]: VFM 释放时活塞/杆上升并使锥套缩径。
- P7 [12]: VFM 定位前撤除释放油压并供给定位油压。
- P8 [12]: VFM 定位时活塞/杆下降并使锥套扩径。
...
- Section: VFM 规格
- Local scope path: VFM > 规格 > 共通使用条件
- Evidence type: TABLE
- Evidence: VFM2000～VFM6000 两组规格表给出相同的精度、压力、温度、气压和流体条件；VFL 规格页给出相同条件。
...
- P1 [20]: 水平姿势采用 `F×0.5/(μ×9.8)`。
- P2 [20]: 水平姿势正确代入 F=700 N 和 μ=0.2。
- P3 [20]: 水平姿势 Gold 为 178.6 kg。
- P4 [20]: 垂直姿势采用 `F×0.5/9.8`。
- P5 [20]: 垂直姿势 Gold 为 35.7 kg。
...
- Gold: 水平 178.6 kg、垂直 35.7 kg；各允许 ±0.1 kg。
...
- P1 [17]: 无符号正确解释为无检测选配的标准型。
- P2 [17]: B 正确解释为着座确认型。
- P3 [17]: B 型法兰上面带有 Z 轴着座。
- P4 [17]: M 正确解释为释放动作确认型。
- P5 [16]: 标准型和 M 型必须另设 Z 轴定位着座面。
- P6 [16]: B 与 M 组合必须另行咨询。
...
- P1 [25]: 空气清洁用供气口始终供气。
- P2 [25]: B 型着座确认用供气口始终供气。
- P3 [25]: M 型释放动作确认用供气口始终供气。
- P4 [25]: 切断清洁供气会让异物侵入并导致动作异常。
...
- P1 [13]: 先将随附 O 形圈装到夹具侧安装孔。
- P2 [13]: 使用强度等级 12.9 的随附内六角螺栓。
- P3 [13]: 螺栓应均匀紧固。
- P4 [13]: 尺寸 2/3 为 M5×0.8、6.3 N·m。
- P5 [12]: 尺寸 4/5 为 M6、10 N·m。
- P6 [12]: 尺寸 6 为 M8、25 N·m。
- P7 [12]: 禁止使用弹簧垫圈。
- P8 [12]: 禁止使用带齿垫圈。
...
- P1 [17]: 倾斜度控制在 4/100～5/100 以内。
- P2 [17]: 该倾斜范围约为 2～3°。
- P3 [17]: 装卸时设置导向销或粗导销。
- P4 [17]: 垂直装卡时避免工件浮起或倾斜。
- P5 [16]: 释放时设置外部预夹紧装置防掉落。
- P6 [16]: 定期确认精度，超限立即更换定位销。
...
- P1 [20]: 先动作 VFL/VFM 扩径定位销。
- P2 [20]: 后动作其他执行元件。
- P3 [20]: 顺序错误会导致精度不良或机器损坏。
- P4 [20]: 回油有背压时使用开启压力小于 0.04 MPa 的单向阀。
- P5 [20]: 调整流量以避免脉冲高压。
...
- P1 [25]: 使用容许偏心量不小于两项间距误差之和的关系。
- P2 [25]: 采用最不利同向叠加。
- P3 [25]: 正确计算 `0.10−0.02=0.08 mm`。
- P4 [25]: Gold 明确为 ±0.08 mm，并完成 0.10 mm 回代核对。
...
- P1 [13]: 配管、接头和夹具流体孔连接前彻底清洗。
- P2 [13]: 防止异物和切削屑进入回路。
- P3 [13]: 在高点/末端等适当位置排出含空气液压油。
- P4 [13]: 必要时设置排气阀。
- P5 [12]: 操作前清除工件和托盘异物。
- P6 [12]: 人员不得进入机器动作范围。
- P7 [12]: 定期检查配管和紧固件是否松动。
- P8 [12]: 定期检查液压油老化并在异常处理后再使用。
