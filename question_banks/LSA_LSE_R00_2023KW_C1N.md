...
- P5 [10]: 正确给出 LSE 未舍入夹紧力 2.8848 kN 和最终结果 2.88 kN。
- P6 [10]: 正确使用 `Fk = 0.953 x P` 并给出未应用上限的 4.5744 kN。
- P7 [15]: 正确应用 3.62 kN 最大保持力，最终回答 3.62 kN。
- P8 [10]: 三项结果均使用 kN，输入压力使用 MPa。
- P9 [15]: 明确采用四舍五入到小数点后两位，并保留要求的未舍入中间值。
...
- Evidence type: FORMULA + TABLE + CHART
...
- P1 [10]: 正确说明曲线条件为 ISO-VG32、25～35°C。
- P2 [20]: 正确说明调节螺钉从关闭侧转向打开侧时流量上升。
- P3 [10]: 正确说明该趋势覆盖三个型号的 A/B 两种回路，而不是只适用于单一曲线。
- P4 [20]: 正确说明同一压力损失下完全打开流量高于完全关闭流量。
- P5 [20]: 正确说明相同型号、回路和开度下，较高压力损失对应较高流量。
- P6 [20]: 正确说明型号、回路、开度和压力损失共同决定读数，拒绝给出脱离条件的通用流量值。
...
- P1 [15]: 明确调整前充分排气。
- P2 [15]: 明确从低速、小流量开始，再缓慢增加到所需速度和流量。
- P3 [20]: 正确说明标准复动式 LSA 的夹紧和释放采用回油节流。
- P4 [20]: 正确说明 LSE 的夹紧和释放采用进油节流，并使用 BZL-A。
- P5 [15]: 正确说明同步控制时每台 LSE 各配一个速度控制阀并分别调整。
- P6 [15]: 正确说明释放方向有负荷时在夹紧油口侧增加回油节流控制。
...
- P1 [15]: 正确给出 4.0 MPa 时夹紧力 2.40 kN、保持力 3.62 kN。
- P2 [15]: 正确区分夹紧力与夹紧后机械自锁机构的保持力边界。
- P3 [20]: 明确反作用力超过保持力会损坏内部部件。
- P4 [15]: 明确刚性不足可在未超过保持力时仍产生位移。
- P5 [15]: 明确无位移要求下反作用力不得超过夹紧力 2.40 kN。
- P6 [10]: 明确禁止夹紧侧与释放侧同时供压。
- P7 [10]: 明确反复在最大保持力额定状态使用会缩短寿命。
...
- Evidence type: TABLE + CHART + CAUTION
...
- P1 [10]: 明确大量空气会使动作时间明显变长，必须排气。
- P2 [15]: 明确排气压力必须降到 2 MPa 以下。
- P3 [15]: 明确从最接近执行器的接头开始，并将螺母旋松约一圈。
- P4 [20]: 明确前后摇动配管，使油液和空气一起排出。
- P5 [15]: 明确排气后重新拧紧接头。
- P6 [12]: 明确对回路最高端进行排气。
- P7 [13]: 明确对从泵看去的最远端进行排气。
...
- P2 [15]: 明确先防止工件掉落和设备误动作。
- P3 [15]: 明确切断液压压力源和电源。
- P4 [15]: 明确确认液压回路压力为零。
- P5 [15]: 明确确认设备和油液已经充分冷却。
- P6 [10]: 明确运转中不得触摸运动部件。
- P7 [10]: 明确不得擅自解体或改造产品。
- P8 [10]: 明确重启前检查并复紧螺栓等连接件，完成安全确认。
...
- P1 [10]: 正确说明 P1 接供油侧。
- P2 [10]: 正确说明 P2 接夹紧器侧。
- P3 [8]: 明确 BZS 无内置过滤器，必须防止切屑、密封带碎屑等污染物进入。
- P4 [8]: 明确供油流量过大会妨碍顺序动作，必须控制流量。
- P5 [8]: 明确空气会造成动作不良，调试前必须排气。
- P6 [10]: 明确出厂未设定压力，必须使用压力表设定。
- P7 [10]: 明确实际供油压力至少比设定压力高 1 MPa。
- P8 [8]: 明确多只 BZS 的相邻设定压力至少相差 1 MPa。
- P9 [8]: 明确设定后至少拧紧一个防转套，力矩为 0.2 N·m。
- P10 [15]: 正确说明阀关闭时夹紧器 1 先动作，达到设定压力后阀打开、夹紧器 2 再动作。
- P11 [5]: 正确说明停压后两者基本同时释放，且一次侧压力下降时止回阀打开。
