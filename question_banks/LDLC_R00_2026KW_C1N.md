...
下表按相邻物理页形成的印刷跨页汇总来源对象。最后一列记录每个来源对象的代表题目，
或说明未保留题目的明确范围决定。

| Coverage ID | 物理页 / 印刷页 | 局部范围 | 证据类型 | 可测试对象 | 代表题目 / 范围决定 |
|---|---|---|---|---|---|
| LDLC-SI-001 | 1-2 / 959-960 | 油压支撑器总览 | TEXT + DRAWING | 支撑器用途、系列选择边界 | 范围决定：总览仅用于限定 LD/LC 边界；共通结构与动作由 `LDLC-Q-0002`、`LDLC-Q-0003` 覆盖，其他系列规格排除 |
| LDLC-SI-002 | 3-6 / 961-964 | LD 特点与动作说明 | TEXT + DRAWING | 接触、锁紧、释放动作及功能 | `LDLC-Q-0002`-`LDLC-Q-0004`、`LDLC-Q-0006` |
| LDLC-SI-003 | 7-8 / 965-966 | LD 型号表示与规格 | TABLE + MODEL | 型号字段、合法组合、支撑力和通用规格 | `LDLC-Q-0007`、`LDLC-Q-0009`、`LDLC-Q-0011`；其余型号列和规格行为同类数值替换，不重复设题 |
| LDLC-SI-004 | 9-12 / 967-970 | LD 能力曲线 | CHART | 油压、柱塞行程与容许负载的视觉读取 | `LDLC-Q-0014`；其他曲线为同类数值替换，保留代表性视觉题 |
| LDLC-SI-005 | 13-16 / 971-974 | LD 标准型与 S 型外形尺寸 | DRAWING + TABLE | 代表型号尺寸和安装要求 | 范围决定：尺寸表主要是型号间数值替换；安装承载原则由 `LDLC-Q-0021` 覆盖，不重复设尺寸题 |
| LDLC-SI-006 | 17-18 / 975-976 | LD-Q 行程加长型 | DRAWING + TABLE | Q 型尺寸、行程和型号边界 | `LDLC-Q-0009` 覆盖 Q 型组合边界；尺寸和行程为同类型号查表，不重复设题 |
| LDLC-SI-007 | 19-22 / 977-980 | LD-E、LD-ES、LD-EQ | DRAWING + TABLE | 弹簧上升系列动作与尺寸 | `LDLC-Q-0004` 覆盖弹簧上升动作；各型号尺寸为同类数值替换，不重复设题 |
| LDLC-SI-008 | 23-26 / 981-984 | LD-M 空气传感器连接型 | STATE_DIAGRAM + TABLE | 传感器气路、检测条件和尺寸 | `LDLC-Q-0006` 覆盖呼吸口、压差和动作确认；传感器选型及尺寸属于外部设备配置，不扩展设题 |
| LDLC-SI-009 | 27-28 / 985-986 | LD 气压清洁与柱塞弹簧设计 | TEXT + DRAWING | 清洁气路、接触螺栓设计 | `LDLC-Q-0019` 覆盖接触螺栓设计；气压清洁为可选气路示例，与传感器/防异物主题重合，不单独设题 |
| LDLC-SI-010 | 29-30 / 987-988 | LC 特点与产品说明 | TEXT + DRAWING | 法兰型结构和功能 | `LDLC-Q-0002`、`LDLC-Q-0003`、`LDLC-Q-0010` |
| LDLC-SI-011 | 31-34 / 989-992 | LC 动作说明 | TEXT + DRAWING | 接触、锁紧、释放动作及配管差异 | `LDLC-Q-0005`、`LDLC-Q-0006`；标准型和弹簧上升型动作与 LD 同构，不重复设题 |
| LDLC-SI-012 | 35-36 / 993-994 | LC 型号表示与规格 | TABLE + MODEL | 字段顺序、合法值、组合限制和规格 | `LDLC-Q-0001`、`LDLC-Q-0008`；其余型号列和规格行为同类数值替换，不重复设题 |
| LDLC-SI-013 | 37-40 / 995-998 | LC 能力曲线与 LC-D | CHART + DRAWING | 曲线视觉读取、无活塞中空型边界 | `LDLC-Q-0015`；其他曲线为同类数值替换，保留代表性视觉题 |
| LDLC-SI-014 | 41-42 / 999-1000 | LC 标准型外形尺寸 | DRAWING + TABLE | 代表型号尺寸和安装要求 | 范围决定：外形尺寸是型号间数值替换，核心规格已由 `LDLC-Q-0008` 代表，不重复设题 |
| LDLC-SI-015 | 43-46 / 1001-1004 | LC-Q 行程加长型外形尺寸 | DRAWING + TABLE | Q 型尺寸、行程和安装要求 | 范围决定：Q 型尺寸和行程是同类型号查表，型号组合边界已在语法中明确，不重复设题 |
| LDLC-SI-016 | 47-50 / 1005-1008 | LC-E、LC-EQ | DRAWING + TABLE | 弹簧上升系列动作与尺寸 | 范围决定：动作与 `LDLC-Q-0004` 的弹簧上升原理同构，尺寸为数值替换，不重复设题 |
| LDLC-SI-017 | 51-52 / 1009-1010 | LC-D 无活塞中空型 | DRAWING + TABLE | 用户自备活塞杆条件和尺寸 | `LDLC-Q-0005`、`LDLC-Q-0020` 覆盖用户自备活塞杆、动作和使用边界；尺寸不重复设题 |
| LDLC-SI-018 | 53-56 / 1011-1014 | LC-M 空气传感器连接型 | STATE_DIAGRAM + TABLE | 传感器气路、检测条件和尺寸 | `LDLC-Q-0006` 覆盖呼吸口、压差和动作确认；传感器外部选型及尺寸不扩展设题 |
| LDLC-SI-019 | 57-58 / 1015-1016 | LC 气压清洁与柱塞弹簧设计 | TEXT + DRAWING | 清洁气路、接触螺栓设计 | `LDLC-Q-0019` 覆盖接触螺栓设计；气压清洁与 LD 同类且与防异物主题重合，不重复设题 |
...
| LDLC-SI-023 | 65-66 / 1725-1726 | 液压油、排气与速度控制 | PROCEDURE + STATE_DIAGRAM | 油液条件、排气步骤和节流方式 | `LDLC-Q-0016` 覆盖通用排气步骤；通用夹紧器回路不迁移到支撑器，支撑器专用速度要求由 `LDLC-Q-0017` 按 SI-021 覆盖 |
| LDLC-SI-024 | 67-68 / 1727-1728 | 操作、维护与保修 | TEXT + PROCEDURE | 安全操作和维护 | `LDLC-Q-0023`；商业保修条款排除 |
| LDLC-SI-025 | 69-70 / 1257-1258 | 控制阀总览 | TABLE | BZL、BZT、BZX、JZG、BZS 功能选择 | `LDLC-Q-0010`、`LDLC-Q-0022` 代表与 LC 直接相关的速度控制和顺序控制；其余阀作为独立附件排除 |
| LDLC-SI-026 | 71-72 / 1259-1260 | BZL 型号与规格 | MODEL + TABLE | G 螺纹、进出油节流、压力范围 | `LDLC-Q-0010` 覆盖 LC 所需进油节流 `BZL□-A`；其他 BZL 规格属于附件扩展，不重复设题 |
| LDLC-SI-027 | 73-74 / 1261-1262 | BZL 外形与流量曲线 | DRAWING + CHART | 尺寸和流量视觉读取 | 范围决定：附件流量曲线不属于 LD/LC 核心能力，且会与两道支撑器能力曲线形成低价值重复，排除 |
| LDLC-SI-028 | 75-76 / 1265-1266 | BZX 排气阀 | MODEL + TABLE + CAUTION | 型号、压力、排气操作风险 | 范围决定：独立附件的手动排气操作不属于 LD/LC 核心产品范围，排除 |
| LDLC-SI-029 | 77-78 / 1267-1268 | JZG 排气阀 | MODEL + TABLE + CAUTION | 型号、压力、排气操作风险 | 范围决定：独立附件的手动排气操作不属于 LD/LC 核心产品范围，排除 |
| LDLC-SI-030 | 79-80 / 1269-1270 | BZS 顺序阀型号与规格 | MODEL + TABLE + CAUTION | 调压范围、压力差、安装和复用限制 | `LDLC-Q-0022` |
| LDLC-SI-031 | 81-82 / 1271-1272 | BZS 外形与动作 | DRAWING + CAUTION | P1/P2 方向、调整螺钉范围和危险 | `LDLC-Q-0024` |
| LDLC-SI-032 | 83-84 / 1273-1274 | LZ-BZS 阀块 | TABLE + DRAWING | 对应 BZS 型号、材料和随附范围 | 范围决定：阀块尺寸、材料和随附范围是独立附件采购信息，排除 |
| LDLC-SI-033 | 85-86 / 1697-1698 | 安装座适用范围 | TABLE | 支撑器与安装座的文档内对应关系 | 范围决定：安装座型号对应属于附件采购映射，不扩展为 LD/LC 产品能力题 |
| LDLC-SI-034 | 87-90 / 1699-1702 | WHZ-MD、LZY-MD、LZ-MS/LZ-MP | TABLE + DRAWING | 安装座与阀块尺寸、质量和对应关系 | 范围决定：独立附件尺寸、质量及对应关系排除 |
| LDLC-SI-035 | 91-96 / 1703-1710 | 配管块与螺母 | TABLE + DRAWING | LZ、TMZ、DZ、WNZ、TNEZ 附件对应关系 | 范围决定：独立附件采购映射排除，不把未列型号关系外推到 LD/LC |
| LDLC-SI-036 | 97-98 / 1729-1730 | 表面粗糙度与 O 形圈标示 | TABLE + MODEL | 新旧标示换算和字段含义 | 范围决定：跨产品通用标示对照不改变本题库答案或判定，排除 |
...
型号表示，且主体尺寸及可否对应表明确列出 LC0553 的 `M-E` 组合，所以该型号符合本资料
明确列出的字段语法。
...
- P2 [10]: 正确说明 `3` 是设计编号。
- P3 [10]: 正确说明 `C` 为板式连接型。
- P4 [5]: 正确说明 `C` 型配有 G 螺纹堵头。
- P5 [5]: 正确说明 `C` 型带排气功能。
- P6 [15]: 正确说明 `L` 为弱弹簧型。
- P7 [15]: 正确说明 `M` 为空气传感器连接型。
- P8 [15]: 正确说明 `E` 为弹簧上升型。
- P9 [10]: 明确判断字段值和顺序符合资料列出的型号语法。
...
- Printed page: 993
- Section: LC 型号表示
- Local scope path: LC > 型号表示 > LC0553-CLM-E 字段；主体尺寸及可否对应 > LC0553 的 M-E 行列
- Evidence type: TABLE + MODEL
- Evidence: 型号表示依次定义主体尺寸、设计编号、配管方式、柱塞弹簧力、动作确认和选配项；同页定义 055、3、C、L、M、E 的含义，主体尺寸及可否对应表在 LC0553 的 `M-E` 行列标为可对应。
...
- P5 [10]: 正确说明弹性垫块使筒夹保持受压。
- P6 [5]: 正确说明该受压状态可避免夹紧过程中的微动。
- P7 [5]: 正确说明该受压状态可防止柱塞与工件之间产生间隙。
...
- Printed page: 966
- Section: LD 规格
- Local scope path: LD > 规格 > 选择无符号 / E 时 > LD0303 列
- Evidence type: TABLE
...
- Printed page: 994
- Section: LC 规格
- Local scope path: LC > 规格 > 选择无符号 / E 时 > LC0403 列
- Evidence type: TABLE
...
- P2 [10]: 正确说明配有 G 螺纹堵头。
- P3 [10]: 正确说明带排气功能。
- P4 [25]: 正确说明 LC 必须选择进油节流型速度控制阀。
- P5 [20]: 正确给出 KOSMEK 对应系列为 `BZL□-A`。
...
- P1 [10]: 正确保留供给油压 7 MPa 条件。
- P2 [10]: 正确保留静载荷 1.0 kN 条件。
- P3 [20]: 正确绑定 LD0303 曲线。
- P4 [40]: 视觉读数落在 6-8 um 范围内。
- P5 [20]: 正确说明曲线排除星号处凹凸和周边夹紧器等造成的工件侧变位。
...
- P1 [10]: 正确保留供给油压 7 MPa 条件。
- P2 [10]: 正确保留静载荷 5.0 kN 条件。
- P3 [20]: 正确绑定 LC0553 曲线。
- P4 [40]: 视觉读数落在 13-17 um 范围内。
- P5 [20]: 正确说明曲线排除星号处凹凸和周边夹紧器等造成的工件侧变位。
...
- P5 [10]: 正确说明在回路最上端和最末端附近排气更有效。
- P6 [10]: 正确说明板式配管应在回路最上端附近设置排气阀。
...
- P2 [10]: 正确选择带单向阀的流量调整阀。
- P3 [10]: 正确选择进油节流方式。
- P4 [20]: 正确给出启开压力 0.1 MPa 以下。
- P5 [20]: 正确说明速度过快会反弹并可能带间隙抱紧。
- P6 [20]: 正确说明启开压力过高会导致释放时柱塞无法复位。
...
- P4 [10]: 正确说明用于保证支撑器呼吸。
- P5 [10]: 正确说明用于减少切削液和异物吸入。
- P6 [20]: 正确说明不设置呼吸孔会导致无法正常呼吸。
...
- P5 [10]: 正确说明缺失或尺寸错误会导致动作不良。
- P6 [10]: 正确说明尺寸错误可能造成支撑器损坏。
...
- P1 [20]: 正确说明无活塞杆供油会使筒夹变形。
- P2 [15]: 正确说明筒夹变形会导致释放不良。
- P3 [15]: 正确说明无活塞杆动作可能损坏筒夹。
- P4 [15]: 正确说明中空状态会让异物或切削液侵入内部。
- P5 [15]: 正确说明内部侵入会造成动作不良。
- P6 [20]: 正确结论是始终安装活塞杆后再放置和使用。
...
- Evidence type: TEXT
- Evidence: 注意事项要求 LD 等外螺纹型支撑器底面与安装孔底面水平密接并由底面承载，并逐项说明邻接螺母、配管座浮起或安装面缝隙会增加变位并造成损坏。
...
- P6 [5]: 正确说明 BZS 内部没有过滤网。
- P7 [5]: 正确说明异物进入会造成顺序阀动作不良。
- P8 [6]: 正确说明内部件受损后即使清除异物也可能无法恢复正常使用。
...
- Printed page: 1269
- Section: BZS 规格 / 注意事项
- Local scope path: BZS > 规格表；注意事项 2-6
- Evidence type: TABLE + TEXT
...
- P1 [15]: 正确说明拆卸前采取防坠落和防误动作措施。
- P2 [15]: 正确说明切断压力源和电源。
- P3 [10]: 正确说明确认油压、气压回路压力为零。
- P4 [10]: 正确说明等待刚停止的设备完全降温。
- P5 [10]: 正确说明定期清扫活塞杆和柱塞周围。
- P6 [15]: 正确说明检查配管、紧固件和夹紧器有无松动并及时加固。
- P7 [10]: 正确说明检查液压油是否老化。
- P8 [7]: 正确说明检查设备有无异音。
- P9 [8]: 正确说明检查动作是否正常顺畅。
...
- Binding: MODEL_FAMILY
- Product: KOSMEK BZS 直装式顺序阀
- Model / Scope: BZS0100 / BZS0200 / BZS0300 油口方向与调压螺钉调整范围

### Question

安装 BZS 直装式顺序阀时，P1、P2 油口应分别连接哪一侧？调整顺序动作压力时，
调压螺钉必须限制在什么范围内，超过该范围继续旋松会有什么危险？

### Standard Answer

P1 油口必须接油压供给侧，P2 油口必须接夹紧器侧。顺序动作压力调节螺钉只能在外形尺寸图所示的
K 值至 C 值范围内调整；到达 `max.C` 位置后不得继续旋松，否则调压螺钉和内部弹簧可能脱落。

### Scoring Standard

- P1 [20]: 正确说明 P1 油口连接油压供给侧。
- P2 [20]: 正确说明 P2 油口连接夹紧器侧。
- P3 [20]: 正确说明调压螺钉只能在 K 值至 C 值范围内调整。
- P4 [20]: 正确说明到达 `max.C` 后不得继续旋松。
- P5 [20]: 正确说明越界旋松可能导致调压螺钉和内部弹簧脱落。

### Accepted Variants

- `油压供给侧` 可写为 `一次侧`，`夹紧器侧` 可写为 `二次侧`，但 P1/P2 对应关系必须正确。
- `K 值至 C 值范围` 可写为 `min.K 至 max.C 的图示调整范围`。

### Forbidden Errors

- 将 P1 接夹紧器侧、P2 接供给侧。
- 声称调压螺钉可超出 `max.C` 继续旋松。
- 忽略调压螺钉或内部弹簧脱落风险。

### Tolerance

- P1/P2 方向及 K-C 调整边界必须精确匹配；无数值容差。
...
- Physical page: 81
- Printed page: 1271
- Section: BZS 外形尺寸 / 安装部位加工尺寸
- Local scope path: BZS > 外形尺寸 > P1/P2 油口；注意事项 > 调压螺钉调整范围
- Evidence type: DRAWING + TEXT
- Evidence: 外形尺寸图将 P1 标为油压供给侧、P2 标为夹紧器侧；图下注释要求调压螺钉在 K 值至 C 值范围内调整，并警告超过 `max.C` 继续旋松可能使调压螺钉和内部弹簧脱落。
