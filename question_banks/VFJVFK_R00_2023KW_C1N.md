...
垂直挂壁或倾斜搬入/搬出工件时，PDF 要求采取哪些防损措施？请分别给出工件与定位销之间的倾斜度控制范围，以及垂直使用时定位销的更换条件。

### Standard Answer

应避免工件浮起或倾斜后直接定位，先用预夹紧装置压紧；垂直使用会使内部滑动面偏磨，应定期确认定位精度，定位精度超出其容许范围时立即更换定位销。工件与扩径定位销之间的倾斜度应控制在 4/100～5/100（约 2～3°）；倾斜状态搬入/搬出尤其易损坏，应设置粗导销引导。
...
- P4 [15]: 正确要求定期确认定位精度。
- P5 [10]: 更换条件正确写为定位精度超出其容许范围。
- P6 [15]: 工件与定位销之间的倾斜度控制范围正确写为 4/100～5/100（约 2～3°）。
...
- 允许倾斜工件直接落到扩径部位，或把 2～3° 倾斜限制误写成定位销的更换阈值。
...
- Physical page: 38
- Printed page: 1396
- Section: 垂直姿势使用；关于工件与扩径定位销的倾斜度
- Local scope path: VFJ/VFK > 注意事项 > 垂直姿势使用 > 定位精度检查与更换；工件倾斜 > 倾斜度与粗导销
- Evidence type: CAUTION + DRAWING
- Evidence: 垂直使用条款要求定期确认定位精度，并在定位精度超出容许范围时更换定位销；另一个倾斜度条款把工件与定位销的倾斜控制在 4/100～5/100（约 2～3°），并要求用粗导销引导搬入/搬出。
...
液压施工投用前应怎样清洁和缠绕密封带？若回路混入大量空气，会产生什么后果并应按什么顺序排气？施工环境还应满足什么清洁要求？

### Standard Answer

配管、接头、夹具油孔等必须彻底清洗后再投入使用；密封带从接头顶部保留 1～2 个螺纹牙再缠绕，且不得让残带进入回路。大量空气会使动作时间异常延长；混入大量空气时，先把供油压力调到 2 MPa 以下，松开离夹紧器/支撑器最近的接头螺母一圈，左右摇动配管排出混气液压油，空气排净后重新紧固，必要时在最高或末端附近设置排气阀。施工环境应保持清洁，防止异物进入回路。
...
- Evidence: 页面逐项列出施工环境清洁、配管/接头/夹具油孔清洗、密封带缠绕、大量空气导致动作时间异常延长，以及五步排气作业。
